...
import { Visibility, Delete, CloudUploadOutlined, CloudDownloadOutlined, AddCircleOutlineSharp, AccountBalanceOutlined, SearchOutlined, UpdateOutlined } from '@material-ui/icons'
...
class InventoryList extends Component {
  ...
  createInventory() {
    this.props.history.push("/create-inventory");
  }
        ...
        <div className="asset-list">
          ...
          <div className="top" style={{ backgroundColor: "#008000" }}>
            ...
            <div className="topRight">
              ...
              <button style={{ marginRight: "8px", margin: "10px", backgroundColor: "#82E0AA", borderColor: "#82E0AA", color: "#D5F5E3" }} className="btn btn-primary float-lg-end" onClick={this.createInventory.bind(this)} >
              ...
              {inventories.length === 0 ?
                ...
                data?.map((inventory, index) => (
                  <tr key={inventory?.id}>
                    ...
                    <td>{inventory.manufactureDate}</td>
                    <td>{inventory.expiryDate}</td>
                    <td>{inventory.unit}</td>
                    <td>{inventory.stockState}</td>
                    <td>{inventory.openingBalance}</td>
                    <td>{inventory.quantityReceived}</td>
                    <td>{inventory.closingStock}</td>
                    <td>{inventory.stockOnHand}</td>
                    <td>{inventory.reportingMonth}</td>
                    <td>{inventory.donor}</td>

                    <td className="text-center" style={{ paddingLeft: "20px" }}><Link to={`/update-inventory/${inventory.id}`} className="edit"><UpdateOutlined /></Link></td>

                    {/* <td className="text-center" style={{ paddingLeft: "20px" }}><Link to={`/update-invent/${inventory.id}`} className="edit"><Edit /></Link></td> */}
                    {
                      users !== 'User' &&
